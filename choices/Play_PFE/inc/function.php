...
function votedBefore($id, $vid){
	...
	try{
		...
		// $q->bindParam(2, $vid);
		// ================ PostgreSQL ================
		$q = $db->prepare("SELECT id FROM votes WHERE id_owner = '$id' AND id_vid = '$vid' ");
		$q->execute();
...
function addVote($v, $id, $vid){
	...
	try{
		...
		// $q->bindParam(3, $vid);
		// ================ PostgreSQL ================
		$q = $db->prepare("INSERT INTO votes( vote, id_owner, id_vid) VALUES ('$v', '$id', '$vid')");
		$q->execute();
	}catch(Exception $e){
		$error_message[] = "Error : ".$e->getMessage();
	}
...
function updateVoted($v, $id, $vid, $voteid){
	...
	try{
		...
		// ================ PostgreSQL ================
		$q = $db->prepare("UPDATE votes SET vote = '$v' ,id_owner = '$id' ,id_vid = '$vid' WHERE id = '$voteid'");
		$q->execute();
	}catch(Exception $e){
		$error_message[] = "Error : ".$e->getMessage();
	}
  ...
  function getVoteCount($id, $v){
  	...
  	try{
		// ================ MySql ================
  		...
  		// $q->bindParam(2, $id);
		// ================ PostgreSQL ================
		$q = $db->prepare("SELECT COUNT(id) FROM votes WHERE vote = '$v' AND id_vid = '$id' ");
  		$q->execute();
  		$cnt = $q->fetch(PDO::FETCH_ASSOC);
		// ================ MySql ================
  		// return $cnt['COUNT(id)'];
